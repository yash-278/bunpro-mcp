...
2. Remote Streamable HTTP accepts the caller's token from `X-Bunpro-Token`, retains `Authorization: Bearer ...` for backwards compatibility, and creates a fresh MCP server for the stateless request.
...
- missing, malformed, ambiguous, and oversized incoming credentials are rejected;
...
The opt-in live tests run the compiled stdio MCP and the Streamable HTTP `X-Bunpro-Token` path with a real `BUNPRO_API_TOKEN`, call `get_connection_status`, and print only normalized status fields.
